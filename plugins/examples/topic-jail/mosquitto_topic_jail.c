...
static int callback_subscribe_unsubscribe(int event, void *event_data, void *userdata)
{
	...
	/* calculate the length of the new payload */
	new_sub_len = strlen(client_id) + sizeof('/') + strlen(ed->subscription) + 1;
	...
	snprintf(new_sub, new_sub_len, "%s/%s", client_id, ed->subscription);

	/* Assign the new topic to the event data structure. You
	 * must *not* free the original topic, it will be handled by the
	 * broker. */
	ed->subscription = new_sub;
...
int mosquitto_plugin_init(mosquitto_plugin_id_t *identifier, void **user_data, struct mosquitto_opt *opts, int opt_count)
{
	...
	if(rc) return rc;
	rc = mosquitto_callback_register(mosq_pid, MOSQ_EVT_SUBSCRIBE, callback_subscribe_unsubscribe, NULL, NULL);
	if(rc) return rc;
	rc = mosquitto_callback_register(mosq_pid, MOSQ_EVT_UNSUBSCRIBE, callback_subscribe_unsubscribe, NULL, NULL);
	return rc;
}
